...
/** Where a node sits relative to the current PR in the neighborhood graph. */
export type GraphRole = 'root' | 'upstream' | 'downstream';

/** A PR node in the neighborhood graph rendered by the Graph view. */
export type GraphNode = {
  readonly ref: PrRef;
  readonly title: string;
  readonly state: PrState;
  readonly role: GraphRole;
  /** On the unmerged path that blocks the root (drives the danger-colored path). */
  readonly blocking: boolean;
};

/**
 * A directed edge in merge order: `from` (a dependency) must merge before `to`
 * (the dependent). The arrow points from `from` → `to`.
 */
export type GraphEdge = { readonly from: PrRef; readonly to: PrRef };

/** The local dependency neighborhood, projected for the Graph view. */
...
export type ResolveResult = {
  ...
  /** The full neighborhood (upstream + this PR + downstream) for the Graph view. */
