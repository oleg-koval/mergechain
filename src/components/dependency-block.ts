...
/** Which representation of the dependencies the block currently shows. */
export type DepView = 'list' | 'graph';
...
export type BlockCallbacks = {
  ...
  /** Switch between the List and Graph representations. */
  readonly onToggleView: (view: DepView) => void;
};
...
const addInput = (current: PrRef, cb: BlockCallbacks): HTMLElement => {
  ...
  const directOption = (ref: PrRef): HTMLElement => {
    ...
    // Context-aware: bare #num same-repo, full owner/repo#num when cross-repo.
...
// Segmented List | Graph control. Reflects the active view and asks the content
// script to switch on click (which flips its state and re-renders the block).
const viewToggle = (view: DepView, cb: BlockCallbacks): HTMLElement => {
  const group = el('div', 'prdeps-viewtoggle');
  group.setAttribute('role', 'tablist');
  (['list', 'graph'] as const).forEach((v) => {
    const btn = el('button', `prdeps-viewbtn${view === v ? ' prdeps-viewbtn--active' : ''}`, v === 'list' ? 'List' : 'Graph');
    btn.type = 'button';
    btn.setAttribute('role', 'tab');
    btn.setAttribute('aria-selected', String(view === v));
    btn.addEventListener('click', () => {
      if (view !== v) cb.onToggleView(v);
    });
...
// The graph is only worth offering once there's an actual relationship to draw
// (more than just this PR on its own).
const isGraphable = (result: ResolveResult): boolean => result.graph.nodes.length > 1;

const tooLargeNote = (count: number): HTMLElement =>
  el(
    'div',
    'prdeps-indirect',
    `Graph too large to display (${count} pull requests); showing the list instead.`,
  );
...
export const createDependencyBlock = (
  ...
  if (state.kind === 'ready' && isGraphable(state.result)) {
    header.appendChild(viewToggle(view, cb));
  }
  const p = pill(state);
  ...
  // Graph view swaps out the list/add-input/dependents; the header, pill,
  // warning, and advisory above stay. Falls back to the list (plus a note) when
  // the graph is too large to draw.
  if (view === 'graph' && isGraphable(result)) {
    const graphEl = createDependencyGraph(current, result.graph);
    if (graphEl) {
      content.appendChild(graphEl);
      block.appendChild(content);
      return block;
    }
    content.appendChild(tooLargeNote(result.graph.nodes.length));
  }
