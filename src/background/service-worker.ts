...
import { reposToSearch } from '../lib/repo-scope.js';
...
// Project the fetched neighborhood into the graph the Graph view renders:
// upstream closure + root (from `snapshot`) as nodes, same-repo dependents as
// downstream nodes, and edges in merge order (dependency → dependent). Edges to
// nodes we couldn't fetch (missing/inaccessible) are dropped so the diagram has
// no dangling endpoints.
const toGraphView = (
  root: PrRef,
  snapshot: ReadonlyMap<string, FetchedPr>,
  dependents: readonly DepStatus[],
  blockingKeys: ReadonlySet<string>,
): GraphView => {
  const rootKey = refKey(root);
  const upstream: readonly GraphNode[] = [...snapshot.values()].map((pr) => ({
    ref: pr.ref,
    title: pr.title,
    state: pr.state,
    role: refKey(pr.ref) === rootKey ? 'root' : 'upstream',
    blocking: blockingKeys.has(refKey(pr.ref)),
  }));
  const upstreamKeys = new Set(upstream.map((n) => refKey(n.ref)));
  const downstream: readonly GraphNode[] = dependents
    .filter((d) => !upstreamKeys.has(refKey(d.ref)))
    .map((d) => ({ ref: d.ref, title: d.title, state: d.state, role: 'downstream', blocking: false }));

  const nodes = [...upstream, ...downstream];
  const present = new Set(nodes.map((n) => refKey(n.ref)));

  const upstreamEdges: readonly GraphEdge[] = [...snapshot.values()].flatMap((pr) =>
    pr.deps
      .filter((dep) => present.has(refKey(dep)) && present.has(refKey(pr.ref)))
      .map((dep) => ({ from: dep, to: pr.ref })),
  );
  const downstreamEdges: readonly GraphEdge[] = downstream.map((d) => ({ from: root, to: d.ref }));

  return { nodes, edges: [...upstreamEdges, ...downstreamEdges], root };
};
...
const handleResolve = async (ref: PrRef): Promise<Result<ResolveResult, AppError>> => {
  ...
  if (!resolved.ok) {
    return ok({
      ...
      transitiveBlocking: [],
      dependents,
      graph: toGraphView(ref, snapshot, dependents, new Set()),
  ...
  return ok({
    ...
    dependents,
    graph: toGraphView(ref, snapshot, dependents, blockingKeys),
    warning: null,
  });
};
...
const handle = async (req: Request): Promise<Response> => {
  switch (req.type) {
    ...
    case 'search-prs': {
      ...
      // Fan out across the current repo + configured allowedRepos so cross-repo
      // PRs are discoverable. A single inaccessible repo is skipped, not fatal;
      // only if EVERY repo fails do we surface the first error.
      const repos = reposToSearch(settings.allowedRepos, { owner: req.owner, repo: req.repo });
      const lists = await Promise.all(repos.map((r) => listOpenPrs(settings.token, r.owner, r.repo)));
      const merged = lists.flatMap((r) => (r.ok ? r.value : []));
      const firstErr = lists.find((r): r is Err<AppError> => !r.ok);
      const result: Result<readonly PrSummaryWire[], AppError> =
        merged.length === 0 && firstErr
          ? firstErr
          : ok(merged.map((pr) => ({ ref: pr.ref, title: pr.title, state: pr.state })));
