...
import { createDependencyBlock, type BlockCallbacks, type BlockState, type DepView } from '../components/dependency-block.js';
...
// List vs Graph view. Session/SPA-scoped (like the caches above): persists
// across GitHub's re-renders, resets to List on a fresh page load or PR switch.
let depView: DepView = 'list';
...
const callbacks = (ref: PrRef): BlockCallbacks => ({
  ...
  onToggleView: (view) => {
    depView = view;
    // Re-render in place with the last resolved state — no refetch needed.
...
const render = (ref: PrRef, state: BlockState): void => {
  lastState = state;
  mountBlock(createDependencyBlock(ref, state, callbacks(ref), depView), currentPlacement);
};
...
const resetPrState = (): void => {
  ...
  currentDeps = [];
  lastBlocked = { blocked: false, reason: '' };
  depView = 'list'; // a new PR starts on the list
  removeBlock();
};
